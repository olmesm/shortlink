...
module ApiKeyRepo =
    ...
    let private selectCols =
        "id, key_hash, name, role, domain_id, enabled, expires_at, created_at"
            ...
            let! id =
                conn.ExecuteScalarAsync<int64>(
                    ...
                    {| keyHash = keyHash
                       ...
                       now = DateTime.UtcNow |})
            let! row =
                conn.QuerySingleAsync<ApiKeyRow>(
                    $"SELECT {selectCols} FROM api_keys WHERE id = @id", {| id = id |})
            return row
        }
        ...
        task {
            ...
            let! rows =
                conn.QueryAsync<ApiKeyRow>(
                    $"SELECT {selectCols} FROM api_keys WHERE key_hash = @keyHash",
                    {| keyHash = keyHash |})
    ...
    let list (db: Db) : Task<ApiKeyRow list> =
        task {
            use conn = db.CreateConnection()
            let! rows =
                conn.QueryAsync<ApiKeyRow>($"SELECT {selectCols} FROM api_keys ORDER BY created_at DESC")
    ...
    let count (db: Db) : Task<int64> =
        task {
            use conn = db.CreateConnection()
            return! conn.ExecuteScalarAsync<int64>("SELECT COUNT(*) FROM api_keys")
        }
